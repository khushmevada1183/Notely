...
import { IToolResult, IToolResultInputOutputDetails, IToolResultOutputDetails, ToolDataSource } from './notely/aiProtocolStubs.js';
...
import { LanguageModelDataPart, LanguageModelTextPart } from './extHostTypes.js';
...
export namespace LanguageModelToolSource {
	export function to(source: Dto<ToolDataSource>): vscode.LanguageModelToolInformation['source'] {
		if (source.type === 'mcp') {
			return new types.LanguageModelToolMCPSource(source.label as string, (source.serverLabel || source.label) as string, source.instructions as string | undefined);
		} else if (source.type === 'extension') {
			return new types.LanguageModelToolExtensionSource(source.extensionId!.value, source.label as string);
		} else {
			return undefined;
		}
	}
...
export namespace LanguageModelToolResult {
	export function to(result: IToolResult): vscode.ExtendedLanguageModelToolResult {
		const toolResult = new types.LanguageModelToolResult(result.content.map(item => {
			if (item.kind === 'text') {
				return new types.LanguageModelTextPart(item.value as string, item.audience as never);
			} else if (item.kind === 'data') {
				const dataItem = item.value as { data: { buffer: ArrayBufferLike }; mimeType: string };
				return new types.LanguageModelDataPart(new Uint8Array(dataItem.data.buffer), dataItem.mimeType, item.audience as never);
			} else {
				return new types.LanguageModelPromptTsxPart(item.value as string);
			}
